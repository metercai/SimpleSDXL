...
def update_after_identity_sub(state):
    ...
    ip_list = modules.flags.ip_list if state["engine"] in ['Fooocus', 'Flux', 'Kolors', 'Comfy', 'Wan', 'Qwen']  else modules.flags.ip_list[:-1]
    ip_list = (ip_list[1:3] + ip_list[-1:]) if state["engine"] in ['Wan', 'Qwen'] else ip_list
