...
def delete_specific_png_files():
    """
    从相对路径查找并删除所有 .png 文件，排除 welcome.png。
    """
    # 获取当前脚本所在目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # 查找 users 目录
    ...
    for root, _, files in os.walk(target_dir):
        for file in files:
            # 查找 .png 文件，排除 welcome.png
            if file.endswith(".png") and file != "welcome.png":
                files_found = True
                file_path = os.path.join(root, file)
                files_to_delete.append(file_path)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(f">>>数字【{Fore.YELLOW}0{Style.RESET_ALL}】+【{Fore.YELLOW}回车{Style.RESET_ALL}】----清理下载/图片缓存与损坏文件<<<     备注：△谨慎执行。慎防误删私有模型")
